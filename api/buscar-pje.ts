...
async function searchDatajud(tribunal: string, processNumber: string, grau?: string) {
  ...
  // DataJud armazena sem formatação (apenas dígitos)
  const numeroSemFormatacao = processNumber.replace(/\D/g, "");

  const query: any = {
    bool: {
      must: [{ term: { "numeroProcesso.keyword": numeroSemFormatacao } }],
    },
  };
...
function buildMovimentosSummary(movimentos: any[], grauLabel: string): string {
  ...
  return movimentos
    .slice(0, 10)
    .map((m: any) => `${m.data}: ${m.nome}${m.complemento ? ` (${m.complemento})` : ""}`)
    .join("\n");
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!process_number) {
      return res.status(400).json({ error: "process_number é obrigatório" });
    }
    ...
    if (process_id) {
      ...
      // Atualizar processo com dados enriquecidos
