...
Com base no contexto do processo fornecido, redija uma MINUTA DE PETIÇÃO profissional e completa em defesa da empresa reclamada.

Regras:
- Use linguagem jurídica formal e precisa
- Fundamente com base na CLT, Constituição Federal, Súmulas/OJs do TST e jurisprudência dos TRTs
- Inclua qualificação das partes, dos fatos, do direito e dos pedidos
- Estruture teses de defesa visando improcedência total ou, subsidiariamente, redução máxima dos valores
- Onde faltar informação, insira marcadores [PREENCHER] ou [VERIFICAR]
- Não invente fatos ou jurisprudência
- Responda em português do Brasil`,

  recurso: `${PERSONA}

Com base no contexto do processo fornecido, redija uma MINUTA DE RECURSO profissional em favor da empresa reclamada.

Regras:
- Identifique os pontos recorríveis com base nos prós e contras do caso
- Use linguagem jurídica formal
- Fundamente com CLT, CF, Súmulas do TST e OJs da SDI
- Estruture: tempestividade, preparo, razões recursais, pedidos
- Foque em reverter condenações ou reduzir valores fixados
- Onde faltar informação, insira [PREENCHER] ou [VERIFICAR]
- Não invente fatos ou jurisprudência
- Responda em português do Brasil`,

  email_cliente: `${PERSONA}

Com base no contexto do processo, redija um E-MAIL PROFISSIONAL para o diretor jurídico ou financeiro da empresa cliente comunicando o andamento do processo.

Regras:
- Use linguagem clara, objetiva e acessível (não excessivamente técnica)
- Resuma o status do processo, riscos, exposição financeira e próximos passos
- Seja transparente sobre pontos positivos e negativos da defesa
- Inclua recomendações práticas para a empresa
- Tom profissional e cordial
- Onde faltar informação, insira [VERIFICAR]
- Responda em português do Brasil`,

  resumo_executivo: `${PERSONA}

Com base no contexto do processo fornecido, produza um RESUMO EXECUTIVO completo e estruturado sob a ótica da defesa da empresa reclamada.

Regras:
- Inclua: identificação do processo, partes, objeto, fase atual
- Resuma os principais pedidos e valores envolvidos
- Apresente os pontos fortes e fracos da defesa
- Indique o nível de risco e justificativa
- Apresente a exposição financeira estimada
- Liste próximos passos e recomendações estratégicas
- Use linguagem jurídica precisa mas compreensível
- Onde faltar informação, insira [VERIFICAR]
- Responda em português do Brasil`,
};
...
serve(async (req) => {
  ...
  try {
    const { type, processContext } = await req.json();

    if (!type || !PROMPTS[type]) {
      return new Response(
        JSON.stringify({ error: `Tipo de documento inválido: ${type}` }),
    ...
    const userContent = processContext
      ? `Com base no seguinte contexto do processo, gere o documento solicitado:\n\n${processContext}`
      : "Gere um modelo genérico do documento solicitado, inserindo [PREENCHER] em todos os campos específicos.";
